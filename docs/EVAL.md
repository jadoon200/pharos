...
| anomaly | trajectory deviation | ROC-AUC (GRU flagship, unsupervised) | 0.962 † |
...
|---|---|---|
| **GRU sequence-AE (flagship, 8 hidden)** | **0.962** | **0.962** |
...
sweep selected 8 hidden units over 16/32/64: 0.963 within / 0.964 cross-region mean AUC, lower
variance, and 804 parameters rather than 38,660 at 64 units. On the 493 real Gulf tracks, the new
and old rankings correlate 0.957 and share 16/25 top outliers; the compact model trains 3.1× faster.
Run `make benchmark-anomaly` to reproduce the capacity sweep independently of the normal eval.

**Leakage and lifecycle audit:** normalization is now fitted only on the training partition, never
on held-out validation tracks. The previously recorded 0.971 / 0.967 result used whole-population
normalization and has been discarded; 0.962 / 0.962 is the corrected five-seed result above. Batch
training atomically writes the fitted weights, normalization statistics, threshold, and provenance
to `data/models/gru-sequence-anomaly.pt`. `POST /score-track` loads that same safe, versioned
tensor/primitives artifact; when it must train a runtime fallback, the response exposes
`model_source=runtime-fallback` rather than silently presenting it as the detector model.
...
| Model | Within-region AUC | Cross-region AUC |
|---|---|---|
| **GRU sequence-AE (flagship)** | **0.962** | **0.962** |
