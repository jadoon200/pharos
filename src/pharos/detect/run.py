...
def run_detectors(positions: list[Position], settings: Settings) -> list[Incident]:
    """Run the four deterministic detectors and return de-duplicated incidents."""
    incidents: list[Incident] = []
    incidents.extend(detect_gaps(positions, settings))
...
def detect(session: Session, region: str | None = None) -> dict[str, int]:
    ...
    incidents = run_detectors(positions, settings)
